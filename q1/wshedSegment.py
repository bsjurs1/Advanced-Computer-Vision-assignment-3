...
def meyers_flooding_algorithm(img, seeds, output_image_path):
    ...
    cv2.imwrite(output_image_path, gradient)

# Utilization of PRIORITY QUEUES USING HEAP QUEUE http://docs.python.org/library/heapq.html is preferable
# HW 3 - Watershed Segmentation
# Write a program to segment the image, present in data\inputImage.jpg with the
# seeds provided in data\inputSeeds.txt. Each row of inputSeeds.txt has
# SEED_ID, X, Y    (for a certain seed pixel).
# You are expected to implement the Meyer flooding algorithm preferably using
# priority queues.
# Arguments:
# inputImageFile - input image file
# inputSeedFile - input seed file (each row corresponds to (SEEDID, X, Y) ) and make sure
# the seeds are as shown in the homework assignment.
...
def main():
    ...
    meyers_flooding_algorithm(img, seeds, output_image_path)
